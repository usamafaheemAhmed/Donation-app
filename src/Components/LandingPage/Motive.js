  ...
  return (
    <div>
      <div className='container mb-4'>
        <div className='row'>
          <div className={`col-lg-7  ${props.Obj.first_block?`${props.Obj.first_block}`:`order-lg-2`}`}>
            <div className='Content_box '>
              <p className='fw-bold' style={{color:"salmon"}}>{props.Obj.slogan} </p>
              <h1 style={{ color: "#042a68" }}>{props.Obj.title}</h1>
              <p>{props.Obj.para2}</p>
              <div className=' w-100'>
                <button type='button' className=' btn btn_Start me-4' onClick={()=>Navigate(props.Obj.link)}> {props.owner?"Explore Portfolio":"Donate Now"}  </button>
          ...
          <div className={`col-lg-5 p-0 m-0 ${props.Obj.second_block?`${props.Obj.second_block}`:`order-lg-1`}`} >
            <div className='Background_Box'>
              ...
              <div>
                <Image src={props.Obj.image} fluid className={`${props.owner?"owner":""}`} />
              </div>
            </div>
          </div>
        </div>
